  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12} sm={6} md={4}>
        <MetricCard label="Загрузка" value={`${data.occupancy_percent.toFixed(1)}%`} helperText="Средняя занятость мест." />
      </Grid>
      <Grid item xs={12} sm={6} md={4}>
        <MetricCard label="Бронирований" value={String(data.bookings_count)} helperText="Общее число операций за период." />
      </Grid>
      <Grid item xs={12} sm={6} md={4}>
        <MetricCard
