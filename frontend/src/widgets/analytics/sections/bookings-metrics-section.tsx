...
import type { AnalyticsBookings, PeakHour } from '../../../shared/types/analytics';
...
export function BookingsMetricsSection({ isLoading, isError, data, peakHours = [] }: { isLoading: boolean; isError: boolean; data?: AnalyticsBookings; peakHours?: PeakHour[] }) {
      ...
      <Grid item xs={12} md={4}>
        <ContentCard sx={{ p: 2, height: '100%', textAlign: 'center' }}>
          ...
          <Stack spacing={0.75} alignItems="center">
            ...
            <Typography variant="body2" color="text.secondary">Неявки: {(data.no_show_rate * 100).toFixed(1)}%</Typography>
          </Stack>
        </ContentCard>
      </Grid>
        ...
        <ContentCard sx={{ p: 2 }}>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
            {peakHours.length === 0 ? (
              ...
              <ContentCard key={`${peakHour.hour}-${peakHour.bookings}`} sx={{ p: 1.5, minWidth: 116, textAlign: 'center' }}>
